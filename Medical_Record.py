...
#AFTER CLICKING REGISTER BUTTON IN FIRST WINDOW
def find1():
    ...
    #AFTER CLICKING CALCULATE BUTTON IN SECOND WINDOW
    def find2():
    ...
    #cursor.execute(sql_command)
            try:
                sql_command = """ create TABLE patient3(name VARCHAR(30), age INTEGER, 
                blood VARCHAR(15), allergy VARCHAR(200), gender VARCHAR(6), bmi INTEGER);"""
                cursor.execute(sql_command)
            except:
                pass
            sql_command = "INSERT INTO patient3(name, age, blood, allergy, gender, bmi) VALUES('{}',{},'{}','{}','{}',{});".format(name, age, blood, allergy, gender, bmi)
            print(sql_command)
            cursor.execute(sql_command)
            cursor.execute("SELECT * FROM patient3")
            ans=cursor.fetchall()
    ...
    def show():
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'O positive'")
        ans=cursor.fetchall()
        a=2
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'A positive'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'B positive'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'AB positive'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'O negative'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'A negative'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'B negative'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
        ...
        cursor.execute("SELECT * FROM patient3 WHERE blood = 'AB negative'")
        ans=cursor.fetchall()
        a=a+1
        for i in ans:
            lb.insert(a,i)
...
Label(main, text="Any Allergies (write 'none' if you don't have any)").grid(row=5)
eName = Entry(main)
eAge = Entry(main)
ePh_No = Entry(main)
...
eAllergy.grid(row=6)
v = IntVar()
Radiobutton(main, text="Male", variable=v, value=1).grid(row=7)
Radiobutton(main, text="Female", variable=v, value=2).grid(row=8)
Label(main, text="").grid(row=9)
btnRegister = Button(main, text="Register", width=20, command=find1) # add a command later
btnRegister.grid(row=10)
main.mainloop()